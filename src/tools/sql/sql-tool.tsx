...
import { SQL_DIALECTS, formatSql, type Dialect } from './sql'
...
export default function SqlTool() {
  ...
  const { output, error } = useMemo(() => {
    ...
    try {
      ...
      return { output: '', error: e instanceof Error ? e.message : t('sql.invalid') }
    }
  }, [input, dialect, t])
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <CodePane
          ...
          autoFocus
        />
        <CodePane label={t('sql.outputLabel')} value={output} error={error} copy rows="lg" />
      </div>
    </div>
  )
}
